...
def reestimate_BN_stats(model, data_loader, num_batches=50, store_ema_stats=False):
    ...
    batch_count = 0
    if BN_flag:
        device = next(model.parameters()).device # 确保 device 已定义
        batch_count = 0 # --- 确保初始化计数器 ---
        with torch.no_grad():
            for x, y in tqdm(data_loader):
                model(x.to(device))
                # We save the running mean/var to a buffer
                # ... 原有的 update 逻辑 (for i, (name, module) in enumerate...) ...
                
                # --- 核心修改：强行在此处加入 break ---
                batch_count += 1
                if batch_count >= 50: 
                    print(f"\n[Quick Check] BN Reestimation finished early at {batch_count} batches.")
                    break
